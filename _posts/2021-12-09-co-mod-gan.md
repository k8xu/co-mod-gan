...
Since co-modulated GANs are generic image-conditional models, they can be easily extended to image-to-image translation tasks. Zhao et al. perform image-to-image translation experiments on the Edges2Handbags dataset and the Edges2Shoes dataset. 

![Edges2Shoes]({{ site.url }}/public/images/2021-12-09-co-mod-gan/fig13.png)

Using a co-modulated GAN results in higher fidelity (FID) over state-of-the-art methods on these edges to photos datasets. The model also produces superior diversity on the Edges2Handbags dataset, but it does not learn to generate diverse images on the Edges2Shoes dataset.
